	...
	static private void DrawBrushGizmo(RaycastHit mouseRayHit,SceneView sceneView)
	{
		...
		else
		{
			if(placingObject != null)
			{
				...
				Handles.ArrowHandleCap(0,placingPlanePoint + ((placingPlanePoint - placingObject.transform.position).normalized * -0.5f),Quaternion.LookRotation(placingPlanePoint - placingObject.transform.position,placingUpVector),1f, EventType.Repaint);
			}
		}
		...
		for(int ii = 0; ii < objects.Length; ii++)
		{
			...
			if(Vector3.Distance(objects[ii].transform.position, newPoint.position) < curTemplate.brushRadius)
			{
				if(eraseSelected == true)
				{
					...
					if (PrefabUtility.GetCorrespondingObjectFromSource(objects[ii]) != (Object)curPrefab)
