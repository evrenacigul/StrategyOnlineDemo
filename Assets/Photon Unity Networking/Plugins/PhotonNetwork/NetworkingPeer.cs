...
#region Operations and Connection Methods
    ...
    public override void Disconnect()
    {
        if (this.PeerState == PeerStateValue.Disconnected)
        {
            if (!PhotonHandler.AppQuits)
            {
                Debug.LogWarning(string.Format("Can't execute Disconnect() while not connected. Nothing changed. AnimState: {0}", this.State));
            }
            return;
        }
    ...
    public void OnOperationResponse(OperationResponse operationResponse)
    {
        ...
        switch (operationResponse.OperationCode)
        {
            ...
            {
                // ClientState oldState = this.AnimState;
            ...
            case OperationCode.JoinGame:
            {
                if (this.Server != ServerConnection.GameServer)
                {
                    ...
                    {
                        if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
                            Debug.Log(string.Format("JoinRoom failed (room maybe closed by now). Client stays on masterserver: {0}. AnimState: {1}", operationResponse.ToStringFull(), this.State));
    ...
    {
        if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
            Debug.Log(string.Format("OnStatusChanged: {0} current AnimState: {1}", statusCode.ToString(), this.State));
    ...
    private void OnSerializeRead(object[] data, PhotonPlayer sender, int networkTime, short correctPrefix)
    {
        ...
        if (view == null)
        {
            Debug.LogWarning("Received OnSerialization for view ID " + viewID + ". We have no such PhotonView! Ignored this if you're leaving a room. AnimState: " + this.State);
            return;
        }
